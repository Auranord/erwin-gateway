...
| Subscriptions | `GET /api/v1/subscriptions`, subscription webhook events | `subscriptions:read` | Includes live subscription EventSub deliveries such as `twitch.channel.subscribe` and gift/message/end variants; any historical repair is manual/operator-owned. |
| Bits | `GET /api/v1/bits/leaderboard`, Bits webhook events | `bits:read` | Includes `twitch.channel.cheer` webhook delivery; any leaderboard repair is manual/operator-owned. |
