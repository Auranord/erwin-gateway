...
Use the [integration guide](README.integration.md) as the source of truth for app registration, bearer API keys, webhook raw-body HMAC verification, idempotency, payload examples, and client examples. Music-specific decisions are:

- Exact filters remain `twitch.chat.message`, `twitch.stream.online`, and `twitch.stream.offline`; do not add `twitch.chat.command` unless the gateway later adds a distinct command event.
- Route `twitch.chat.message` events with `chat.is_command = true` to existing handlers for `!vote`, `!song`, `!skip`, `!pause`, and `!resume`.
- Keep music-domain authorization, queue effects, vote accounting, skip/pause/resume behavior, overlay updates, Discord notifications, and chat response decisions in `erwin-music`.
- Move only static text commands such as `!dc`, `!discord`, `!youtube`, `!socials`, `!commands`, and `!lurk` into the gateway Text Commands UI when they do not need music state.
- Replace IRC sends with `POST /api/v1/chat/messages` and stable `idempotency_key` values; see [send-chat examples](README.integration.md#62-send-chat-with-a-stable-idempotency_key).
- Replace direct stream-status polling with `GET /api/v1/streams/current` or stream online/offline webhooks when event-driven behavior is enough.
- For webhook receiver requirements and duplicate handling, follow [webhook verification](README.integration.md#5-receive-and-verify-webhooks) and [idempotency expectations](README.integration.md#idempotency-expectations).

## Failure behavior

- Gateway retries signed webhooks and dead-letters permanently failed deliveries; inspect app-owned delivery diagnostics with `/api/v1/webhook-deliveries` or the Admin UI.
