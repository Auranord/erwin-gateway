...
export async function registerAdminApiRoutes(app: FastifyInstance, options: AdminRouteOptions) {
  ...
  app.get('/api/admin/channel-points', async (request, reply) => {
    ...
    const query = z.object({
      ...
      redemptionQ: z.string().optional(),
      redemptionStatus: z.string().optional(),
      redemptionLimit: z.coerce.number().int().positive().optional()
    }).safeParse(request.query);
    if (!query.success) return reply.code(400).send({ error: 'Invalid query', issues: query.error.issues });
    const rewards = await listRewards(options.db, adminChannelPointApp, { includeDeleted: query.data.includeDeleted === 'true' });
    const redemptions = await listRedemptions(options.db, adminChannelPointApp, { q: query.data.redemptionQ, status: query.data.redemptionStatus, limit: query.data.redemptionLimit ?? 50 });
