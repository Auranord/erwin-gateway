...
export function validateCommandPrefix(value?: string) {
  const prefix = normalizePrefix(value);
  const issues: string[] = [];
  if (prefix.length > 8) issues.push('commandPrefix must be 1-8 characters');
  return { issues, prefix };
}

async function configuredCommandPrefix(db: Database, channelId?: string | null) {
  const [channel] = channelId
    ? await db.select().from(twitchChannels).where(eq(twitchChannels.id, channelId)).limit(1)
    : await db.select().from(twitchChannels).where(eq(twitchChannels.primaryChannel, true)).limit(1);
  return normalizePrefix(channel?.commandPrefix);
}

export async function getPrimaryChannelCommandPrefix(db: Database) {
  const [channel] = await db.select().from(twitchChannels).where(eq(twitchChannels.primaryChannel, true)).limit(1);
  return channel ? { ok: true as const, channel, commandPrefix: normalizePrefix(channel.commandPrefix) } : { ok: false as const, statusCode: 404, error: 'Primary Twitch channel not found' };
}

export async function updatePrimaryChannelCommandPrefix(db: Database, commandPrefix: string) {
  const { issues, prefix } = validateCommandPrefix(commandPrefix);
  if (issues.length) return { ok: false as const, statusCode: 400, error: 'Invalid command prefix', issues };
  const [channel] = await db.update(twitchChannels).set({ commandPrefix: prefix, updatedAt: new Date() }).where(eq(twitchChannels.primaryChannel, true)).returning();
  return channel ? { ok: true as const, channel, commandPrefix: normalizePrefix(channel.commandPrefix) } : { ok: false as const, statusCode: 404, error: 'Primary Twitch channel not found' };
}

export function validateTextCommandInput(input: { command?: string; aliases?: string[]; responseText?: string; requiredRole?: string; replyMode?: string; cooldownSeconds?: number; userCooldownSeconds?: number }, commandPrefix = '!') {
  const prefix = normalizePrefix(commandPrefix);
...
export async function createTextCommand(db: Database, input: UpsertInput) {
  const commandPrefix = await configuredCommandPrefix(db, input.channelId);
  const { issues, normalized } = validateTextCommandInput(input, commandPrefix);
...
export async function updateTextCommand(db: Database, id: string, input: Partial<UpsertInput>) {
  ...
  const commandPrefix = await configuredCommandPrefix(db, input.channelId === undefined ? existing.channelId : input.channelId);
  const { issues, normalized } = validateTextCommandInput(merged, commandPrefix);
  if (issues.length) return { ok: false as const, statusCode: 400, error: 'Invalid text command', issues };
...
async function findMatchingCommand(db: Database, input: ExecuteInput) {
  ...
  const prefix = await configuredCommandPrefix(db, input.channelId);
